...
import static lei.com.example.wilocclient.SensorTagGatt.*;

import static java.lang.Math.pow;

import java.util.List;
import java.util.UUID;
import lei.com.example.wilocclient.Point3D;
...
public enum Sensor {
  ...
  MAGNETOMETER(UUID_MAG_SERV, UUID_MAG_DATA, UUID_MAG_CONF) {
    @Override
    public Point3D convert(final byte [] value) {
    	Point3D mcal = new Point3D(0.0,0.0,0.0);
  ...
  GYROSCOPE(UUID_GYR_SERV, UUID_GYR_DATA, UUID_GYR_CONF, (byte)7) {
    ...
    public Point3D convert(final byte [] value) {
      ...
      float y = shortSignedAtOffset(value, 0) * (2000f / 65536f) * -1;
      float x = shortSignedAtOffset(value, 2) * (2000f / 65536f);
      float z = shortSignedAtOffset(value, 4) * (2000f / 65536f);
